...
export default function Titular7({ posição, title }){

    const { 
        larguraTela, defensoresConvocados, meiasConvocados, atacantesConvocados, 
        titular7, setTitular7, player, titular2, titular3, titular4,
        titular5, titular6, titular8, titular9, titular10, titular11
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const t7Storage = localStorage.getItem('titular7');
        
            if(t7Storage){
                setTitular7(JSON.parse(t7Storage));
            }
            if(t7Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('titular7', JSON.stringify(titular7));
      }, [titular7]);

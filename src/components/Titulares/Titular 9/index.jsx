...
export default function Titular9({ posição, title }){

    const { 
        larguraTela, defensoresConvocados, meiasConvocados, atacantesConvocados, 
        titular9, setTitular9, player, titular2, titular3, titular4,
        titular5, titular6, titular8, titular7, titular10, titular11
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const t9Storage = localStorage.getItem('titular9');
        
            if(t9Storage){
                setTitular9(JSON.parse(t9Storage));
            }
            if(t9Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('titular9', JSON.stringify(titular9));
      }, [titular9]);

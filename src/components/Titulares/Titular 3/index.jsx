...
export default function Titular3({ posição, title }){

    const { 
        larguraTela, defensoresConvocados, meiasConvocados, atacantesConvocados, titular3, setTitular3, player
    ...
    useEffect(() => {
        const t3Storage = localStorage.getItem('titular3');
    
        if(t3Storage){
            setTitular3(JSON.parse(t3Storage));
        }
        if(t3Storage.length === 2){
            setSelect(false);
        }else{
            setSelect(true);
        }
      ...
      useEffect(() => {
        localStorage.setItem('titular3', JSON.stringify(titular3));
      }, [titular3]);

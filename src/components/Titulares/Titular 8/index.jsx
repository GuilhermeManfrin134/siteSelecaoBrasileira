...
export default function Titular8({ posição, title }){

    const { 
        larguraTela, defensoresConvocados, meiasConvocados, atacantesConvocados, 
        titular8, setTitular8, player, titular2, titular3, titular4,
        titular5, titular6, titular7, titular9, titular10, titular11
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const t8Storage = localStorage.getItem('titular8');
        
            if(t8Storage){
                setTitular8(JSON.parse(t8Storage));
            }
            if(t8Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('titular8', JSON.stringify(titular8));
      }, [titular8]);

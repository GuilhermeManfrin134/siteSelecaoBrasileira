...
export default function Titular10({ posição, title }){

    const { 
        larguraTela, defensoresConvocados, meiasConvocados, atacantesConvocados, 
        titular10, setTitular10, player, titular2, titular3, titular4,
        titular5, titular6, titular8, titular7, titular9, titular11
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const t10Storage = localStorage.getItem('titular10');
        
            if(t10Storage){
                setTitular10(JSON.parse(t10Storage));
            }
            if(t10Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('titular10', JSON.stringify(titular10));
      }, [titular10]);

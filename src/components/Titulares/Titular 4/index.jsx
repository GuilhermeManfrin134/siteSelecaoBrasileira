...
export default function Titular4({ posição, title }){

    const { 
        larguraTela, defensoresConvocados, meiasConvocados, atacantesConvocados, titular4, setTitular4, player
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const t4Storage = localStorage.getItem('titular4');
        
            if(t4Storage){
                setTitular4(JSON.parse(t4Storage));
            }
            if(t4Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('titular4', JSON.stringify(titular4));
      }, [titular4]);

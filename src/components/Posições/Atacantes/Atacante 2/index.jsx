...
export default function Atacante2(){

    const { larguraTela, player, atacante2, setAtacante2, TextoAtacantes, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const a2Storage = localStorage.getItem('atacante2');
        
            if(a2Storage){
                setAtacante2(JSON.parse(a2Storage));
            }
            if(a2Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('atacante2', JSON.stringify(atacante2));
      }, [atacante2]);

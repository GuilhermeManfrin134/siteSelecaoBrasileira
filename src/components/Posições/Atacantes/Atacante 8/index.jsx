...
export default function Atacante6(){
    ...
    const { larguraTela, player, atacante8, setAtacante8, TextoAtacantes, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const a8Storage = localStorage.getItem('atacante8');
        
            if(a8Storage){
                setAtacante8(JSON.parse(a8Storage));
            }
            if(a8Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('atacante8', JSON.stringify(atacante8));
      }, [atacante8]);

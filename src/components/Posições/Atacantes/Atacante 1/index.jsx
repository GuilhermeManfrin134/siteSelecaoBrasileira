...
export default function Atacante1(){
    ...
    const { larguraTela, player, atacante1, setAtacante1, TextoAtacantes, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const a1Storage = localStorage.getItem('atacante1');
        
            if(a1Storage){
                setAtacante1(JSON.parse(a1Storage));
            }
            if(a1Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('atacante1', JSON.stringify(atacante1));
      }, [atacante1]);

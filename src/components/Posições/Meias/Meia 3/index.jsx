...
export default function Meia3(){
    ...
    const { larguraTela, player, meia3, setMeia3, TextoMeias, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const m3Storage = localStorage.getItem('meia3');
        
            if(m3Storage){
                setMeia3(JSON.parse(m3Storage));
            }
            if(m3Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('meia3', JSON.stringify(meia3));
      }, [meia3]);

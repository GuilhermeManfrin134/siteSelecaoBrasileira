...
export default function Meia4(){

    const { larguraTela, player, meia4, setMeia4, TextoMeias, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);

    const [select, setSelect] = useState(false);
    const [playerModal, setPlayerModal] = useState('');

    useEffect(() => {
        const m4Storage = localStorage.getItem('meia4');
    
        if(m4Storage){
            setMeia4(JSON.parse(m4Storage));
        }
        if(m4Storage.length === 2){
            setSelect(false);
        }else{
            setSelect(true);
        }
      ...
      useEffect(() => {
        localStorage.setItem('meia4', JSON.stringify(meia4));
      }, [meia4]);

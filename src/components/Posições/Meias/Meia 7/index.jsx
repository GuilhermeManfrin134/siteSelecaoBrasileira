...
export default function Meia7(){

    const { larguraTela, player, meia7, setMeia7, TextoMeias, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const m7Storage = localStorage.getItem('meia7');
        
            if(m7Storage){
                setMeia7(JSON.parse(m7Storage));
            }
            if(m7Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('meia7', JSON.stringify(meia7));
      }, [meia7]);

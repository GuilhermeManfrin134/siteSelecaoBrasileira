...
export default function Meia2(){

    const { larguraTela, player, meia2, setMeia2, TextoMeias, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        if(componentMounted.current){
            const m2Storage = localStorage.getItem('meia2');
        
            if(m2Storage){
                setMeia2(JSON.parse(m2Storage));
            }
            if(m2Storage.length === 2){
                setSelect(false);
            }else{
                setSelect(true);
            }
      ...
      useEffect(() => {
        localStorage.setItem('meia2', JSON.stringify(meia2));
      }, [meia2]);

...
export default function Defensor4(){

    const { larguraTela, player, defensor4, setDefensor4, TextoDefensores, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);
    ...
    useEffect(async () => {
        const d4Storage = localStorage.getItem('defensor4');
        
        if(d4Storage){
            setDefensor4(JSON.parse(d4Storage));
        }
        if(d4Storage.length === 2){
            setSelect(false);
        }else{
            setSelect(true);
        }
      ...
      useEffect(() => {
        localStorage.setItem('defensor4', JSON.stringify(defensor4));
      }, [defensor4]);

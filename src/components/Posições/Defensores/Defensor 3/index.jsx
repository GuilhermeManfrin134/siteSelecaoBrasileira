...
export default function Defensor3(){

    const { larguraTela, player, defensor3, setDefensor3, TextoDefensores, defensoresConvocados, meiasConvocados, atacantesConvocados } = useContext(UserContext);

    const [select, setSelect] = useState(false);
    const [playerModal, setPlayerModal] = useState('');

    useEffect(() => {
        const d3Storage = localStorage.getItem('defensor3');
        
        if(d3Storage){
            setDefensor3(JSON.parse(d3Storage));
        }
        if(d3Storage.length === 2){
            setSelect(false);
        }else{
            setSelect(true);
        }
      ...
      useEffect(() => {
        localStorage.setItem('defensor3', JSON.stringify(defensor3));
      }, [defensor3]);
